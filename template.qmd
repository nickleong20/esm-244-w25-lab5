...
```{r}
seeds_df_longer <- seeds_df %>%
  pivot_longer(cols = c(a, p, c, l_k, w_k, asym, l_g))

ggplot(seeds_df_longer, aes(x = value)) +
  geom_histogram() +
...
```{r}
ggplot(seeds_df, aes(x = l_g, y = w_k, color = c, shape = variety)) +
  geom_point() +
  theme_minimal()
...
```{r}
#| fig-cap: We would 3 groups to initate a kmeans cluster beacuse the kink in the graph is most pronounced at the three clusters
...
# We think the min is around 2, max is around 10. This gives us the recommended number of clusters
...
set.seed(10101)
seeds_km <- kmeans(seeds_scale, 3, 25) # kmeans specifying 3 groups to start
```
...
```{r}
seeds_cl <- seeds_complete %>%
  mutate(cluster = seeds_km$cluster)
...
ggplot(seeds_cl) +
  geom_point(aes(x = a, y = asym, color = cluster_no, shape = variety), size = 2) +
  theme_bw()
...
```{r}
#| tbl-cap: Contiency table for the grouping of kmeans and the varietals of the kernels 
...
```{r}
library(patchwork)
combined <- p_complete + p_single + plot_layout(ncol =2)
combined
```
